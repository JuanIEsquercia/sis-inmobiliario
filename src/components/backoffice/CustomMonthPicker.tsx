"use client";

import { useEffect, useRef, useState } from "react";
import { useRouter } from "next/navigation";

const monthNames = [
  "Enero", "Febrero", "Marzo", "Abril", "Mayo", "Junio",
  "Julio", "Agosto", "Septiembre", "Octubre", "Noviembre", "Diciembre",
];
const monthAbbr = ["Ene", "Feb", "Mar", "Abr", "May", "Jun", "Jul", "Ago", "Sep", "Oct", "Nov", "Dic"];

// Piloto: reemplaza el <input type="month"> nativo (cuyo popup lo dibuja
// el sistema operativo, imposible de restylear) por un panel propio —
// mismo trigger visual que MonthPicker, pero el calendario que se abre
// es HTML/CSS nuestro. Si funciona bien acá, se generaliza al resto de
// los calendarios de la app; por ahora vive aparte para no tocar los
// otros usos de MonthPicker mientras se evalúa.
// Bloque de 12 años que contiene a `y` — alineado a múltiplos de 12
// para que paginar hacia atrás/adelante siempre caiga en los mismos
// cortes y no "derive" según desde dónde se abrió.
const YEARS_PER_PAGE = 12;
function yearBlockStart(y: number) {
  return Math.floor(y / YEARS_PER_PAGE) * YEARS_PER_PAGE;
}

export function CustomMonthPicker({ month, year, basePath }: { month: number; year: number; basePath: string }) {
  const router = useRouter();
  const [open, setOpen] = useState(false);
  const [viewYear, setViewYear] = useState(year);
  // Dos vistas dentro del mismo panel, como cualquier calendario
  // conocido: se abre en meses, y tocando el año se pasa a la grilla de
  // años para saltar directo. Antes el año solo se movía de a uno con
  // las flechas — para ir a un período de hace dos años había que
  // clickear dos veces la flecha (o, en Liquidaciones, que ni siquiera
  // tenía este panel, ~24 veces "Anterior").
  const [view, setView] = useState<"months" | "years">("months");
  const [alignRight, setAlignRight] = useState(false);
  const [openUpwards, setOpenUpwards] = useState(false);
  const containerRef = useRef<HTMLDivElement>(null);

  useEffect(() => {
    if (!open) return;

    function onPointerDown(e: MouseEvent) {
      if (containerRef.current && !containerRef.current.contains(e.target as Node)) {
        setOpen(false);
      }
    }
    function onKeyDown(e: KeyboardEvent) {
      if (e.key === "Escape") setOpen(false);
    }
    document.addEventListener("mousedown", onPointerDown);
    document.addEventListener("keydown", onKeyDown);
    return () => {
      document.removeEventListener("mousedown", onPointerDown);
      document.removeEventListener("keydown", onKeyDown);
    };
  }, [open]);

  function toggleOpen() {
    setViewYear(year);
    setView("months");
    if (!open && containerRef.current) {
      const rect = containerRef.current.getBoundingClientRect();
      const viewportWidth = window.innerWidth;
      const viewportHeight = window.innerHeight;
      setAlignRight(viewportWidth - rect.left < 300 || rect.left > viewportWidth / 2);
      setOpenUpwards(viewportHeight - rect.bottom < 260 && rect.top > 260);
    }
    setOpen((o) => !o);
  }

  function selectMonth(m: number) {
    setOpen(false);
    router.push(`${basePath}?mes=${m}&anio=${viewYear}`);
  }

  return (
    <div ref={containerRef} className="relative inline-flex">
      <button
        type="button"
        onClick={toggleOpen}
        aria-haspopup="dialog"
        aria-expanded={open}
        className="flex items-center gap-2.5 rounded-xl border border-border/70 bg-surface px-3.5 py-2 text-xs font-bold text-foreground shadow-xs transition-all hover:border-accent hover:bg-accent-soft/30 cursor-pointer"
      >
        <svg className="h-4 w-4 flex-none text-accent" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.2">
          <rect x="3" y="4" width="18" height="18" rx="2" ry="2" />
          <line x1="16" y1="2" x2="16" y2="6" />
          <line x1="8" y1="2" x2="8" y2="6" />
          <line x1="3" y1="10" x2="21" y2="10" />
        </svg>
        <span>
          {monthNames[month - 1]} {year}
        </span>
        <svg
          className={`ml-1 h-3.5 w-3.5 flex-none text-muted transition-transform ${open ? "rotate-180" : ""}`}
          fill="none"
          viewBox="0 0 24 24"
          stroke="currentColor"
          strokeWidth="2"
        >
          <path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" />
        </svg>
      </button>

      {open && (
        <div
          role="dialog"
          aria-label="Elegir mes"
          className={`absolute z-50 w-72 max-w-[90vw] rounded-3xl border border-border/70 bg-surface/95 p-4 shadow-premium backdrop-blur-md animate-fadeIn ${
            openUpwards ? "bottom-[calc(100%+0.5rem)] top-auto" : "top-[calc(100%+0.5rem)] bottom-auto"
          } ${alignRight ? "right-0 left-auto" : "left-0 right-auto"}`}
        >
          <div className="mb-3 flex items-center justify-between border-b border-border/40 pb-2.5">
            <button
              type="button"
              onClick={() => (view === "months" ? setViewYear((y) => y - 1) : setViewYear((y) => y - YEARS_PER_PAGE))}
              aria-label={view === "months" ? "Año anterior" : "Años anteriores"}
              className="flex h-8 w-8 items-center justify-center rounded-xl border border-border/60 text-muted transition-colors hover:bg-accent-soft/40 hover:text-accent cursor-pointer"
            >
              <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.3">
                <path strokeLinecap="round" strokeLinejoin="round" d="M15 19l-7-7 7-7" />
              </svg>
            </button>

            {/* El título es el que cambia de vista: en meses muestra el
                año y lleva a la grilla de años; en años muestra el rango
                y vuelve a meses. */}
            <button
              type="button"
              onClick={() => setView((v) => (v === "months" ? "years" : "months"))}
              aria-label={view === "months" ? "Elegir año" : "Volver a los meses"}
              className="flex items-center gap-1.5 rounded-xl px-2.5 py-1 text-base font-extrabold tracking-tight text-foreground transition-colors hover:bg-accent-soft/50 hover:text-accent cursor-pointer"
            >
              {view === "months"
                ? viewYear
                : `${yearBlockStart(viewYear)} – ${yearBlockStart(viewYear) + YEARS_PER_PAGE - 1}`}
              <svg
                className={`h-3.5 w-3.5 text-muted transition-transform ${view === "years" ? "rotate-180" : ""}`}
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
                strokeWidth="2.3"
              >
                <path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" />
              </svg>
            </button>

            <button
              type="button"
              onClick={() => (view === "months" ? setViewYear((y) => y + 1) : setViewYear((y) => y + YEARS_PER_PAGE))}
              aria-label={view === "months" ? "Año siguiente" : "Años siguientes"}
              className="flex h-8 w-8 items-center justify-center rounded-xl border border-border/60 text-muted transition-colors hover:bg-accent-soft/40 hover:text-accent cursor-pointer"
            >
              <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.3">
                <path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7" />
              </svg>
            </button>
          </div>

          {view === "months" ? (
            <div className="grid grid-cols-3 gap-2">
              {monthAbbr.map((label, idx) => {
                const m = idx + 1;
                const isSelected = m === month && viewYear === year;
                return (
                  <button
                    key={m}
                    type="button"
                    onClick={() => selectMonth(m)}
                    aria-current={isSelected ? "true" : undefined}
                    className={`rounded-xl px-2 py-2.5 text-xs font-bold transition-all cursor-pointer ${
                      isSelected
                        ? "bg-accent text-accent-foreground shadow-sm scale-95"
                        : "text-foreground hover:bg-accent-soft/50"
                    }`}
                  >
                    {label}
                  </button>
                );
              })}
            </div>
          ) : (
            <div className="grid grid-cols-3 gap-2">
              {Array.from({ length: YEARS_PER_PAGE }, (_, i) => yearBlockStart(viewYear) + i).map((y) => {
                const isSelected = y === year;
                return (
                  <button
                    key={y}
                    type="button"
                    // Elegir el año no navega todavía: vuelve a la
                    // grilla de meses ya parado en ese año, para que el
                    // salto sea "año + mes" en dos toques y una sola
                    // recarga de página.
                    onClick={() => {
                      setViewYear(y);
                      setView("months");
                    }}
                    aria-current={isSelected ? "true" : undefined}
                    className={`rounded-xl px-2 py-2.5 text-xs font-bold transition-all cursor-pointer ${
                      isSelected
                        ? "bg-accent text-accent-foreground shadow-sm scale-95"
                        : "text-foreground hover:bg-accent-soft/50"
                    }`}
                  >
                    {y}
                  </button>
                );
              })}
            </div>
          )}

          <button
            type="button"
            onClick={() => {
              const now = new Date();
              setOpen(false);
              router.push(`${basePath}?mes=${now.getUTCMonth() + 1}&anio=${now.getUTCFullYear()}`);
            }}
            className="mt-3 w-full rounded-xl border border-border/60 py-2 text-[11px] font-bold uppercase tracking-wider text-muted transition-colors hover:border-accent hover:bg-accent-soft/30 hover:text-accent cursor-pointer"
          >
            Ir al mes actual
          </button>
        </div>
      )}
    </div>
  );
}
