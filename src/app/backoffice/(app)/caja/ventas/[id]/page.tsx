...
import { marcarCuotaPagada } from "../../actions";

const fmtDate = new Intl.DateTimeFormat("es-AR", { dateStyle: "medium" });
const fmtMoney = (n: number) => n.toLocaleString("es-AR", { maximumFractionDigits: 2 });

const pagareSignerLabels: Record<string, string> = {
  COMPRADOR: "Comprador",
  VENDEDOR: "Vendedor",
};
          ...
          <tbody>
            {sale.installments.map((installment) => (
              <tr key={installment.id} className="border-b border-border last:border-0">
                ...
                <td className="px-4 py-2.5 text-muted">
                  {installment.pagareSignedBy ? pagareSignerLabels[installment.pagareSignedBy] : "Sin pagaré"}
                </td>
                <td className="px-4 py-2.5 text-muted">
                ...
                {canCollect && (
                  <td className="px-4 py-2.5">
                    {installment.status !== "PAGADA" && (
                      <form action={marcarCuotaPagada.bind(null, installment.id)}>
                        <button
                          type="submit"
